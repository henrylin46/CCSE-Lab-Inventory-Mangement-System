        ...
        if($stockStatement->rowCount() > 0){
            ...
            if($currentQuantityInItemsTable <= 0) {
                ...
                if($customerStatement->rowCount() > 0){
                    ...
                    $insertSaleSql = 'INSERT INTO sale(itemNumber, itemName, quantity, customerID, customerName, saleDate, purpose, requestStatus) VALUES(:itemNumber, :itemName, :quantity, :customerID, :customerName, :saleDate, :purpose, :requestStatus)';
                    $insertSaleStatement = $conn->prepare($insertSaleSql);
                    $insertSaleStatement->execute(['itemNumber' => $itemNumber, 'itemName' => $itemName, 'quantity' => $quantity, 'customerID' => $customerID, 'customerName' => $customerName, 'saleDate' => $saleDate, 'purpose' => $purpose, 'requestStatus' => $requestStatus]);
